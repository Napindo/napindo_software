...
type NavLeaf = { label: string; icon: IconName; active?: boolean }
type NavParent = {
  label: string
  icon: IconName
...
const navStructure: { title?: string; items: NavItem[] }[] = [
  {
    items: [
      { label: 'Dashboard', icon: 'grid', active: true },
      {
        ...
        icon: 'pencil',
        items: [
          { label: 'Exhibitor', icon: 'tag' },
          { label: 'Visitor', icon: 'chart' },
          { label: 'Import Data', icon: 'tag' },
        ],
      },
      {
        label: 'Print Label',
    ...
    items: [
      {
        ...
        items: [
          { label: 'Perusahaan', icon: 'building' },
          { label: 'Government', icon: 'shield' },
          { label: 'Jumlah Perusahaan', icon: 'chart' },
          { label: 'Jumlah Government', icon: 'chart' },
        ],
      },
    ],
  },
                ...
                {group.items.map((item) => {
                        ...
                        >
                          {parent.items.map((child) => (
                            <button
                              key={child.label}
                              type="button"
                              className="w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg text-slate-700 hover:bg-rose-50 transition"
                            >
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.active ? <span className="w-1 h-8 rounded-full bg-white/80" aria-hidden /> : null}
                    </button>
                  )
                })}
              </div>
